...
function tmpPathFor(configPath) {
...
function acquire(lockPath, timeoutMs, staleTTLMs) {
    ...
    for (;;) {
        try {
            ...
            if (err.code !== "EEXIST")
                throw err;
            if (reclaimIfStale(lockPath, staleTTLMs))
                continue;
            if (Date.now() >= deadline) {
                throw new Error(`withLockedFile: timed out acquiring ${lockPath} after ${timeoutMs}ms`);
            }
...
function reclaimIfStale(lockPath, staleTTLMs) {
    let ageMs;
    try {
...
export function withLockedFile(configPath, fn, opts = {}) {
    const lockPath = lockPathFor(configPath);
    acquire(lockPath, opts.acquireTimeoutMs ?? defaultAcquireTimeoutMs, opts.staleTTLMs ?? lockStaleTTLMs);
    try {
        return fn();
    }
    finally {
...
export function atomicWriteFileSync(configPath, data, mode = 0o600) {
