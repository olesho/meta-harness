...
export function spoolFilePath(spoolDir) {
...
export function appendSpool(spoolDir, events) {
    if (events.length === 0)
        return;
...
// aborting the drain.
export function drainSpool(spoolDir) {
    const file = spoolFilePath(spoolDir);
    if (!existsSync(file))
        return [];
...
function parseLines(raw) {
    ...
    for (const line of raw.split("\n")) {
        ...
        if (rec.parentSessionID)
            pe.parentSessionID = rec.parentSessionID;
        out.push(pe);
    }
    return out;
}
...
function rehydrateEvent(raw) {
    const event = { ...raw, source: SourceHook };
