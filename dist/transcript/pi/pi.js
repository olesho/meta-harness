...
import { cleanPosix } from "../pathutil.js";
export class PiReader {
    ...
    sessionsDir() {
        ...
        if (this.sessionsDir_ !== "")
            return this.sessionsDir_;
        if (this.root !== "")
    ...
    locate(sessionID, workingDir) {
        ...
        try {
            ...
            if (err?.code === "ENOENT") {
                throw wrap(`pi transcript: no session file for ${sessionID} under ${sessionsDir}`, ErrSessionNotFound);
            }
...
function findInDir(dir, sessionID) {
    let entries;
    try {
...
// matches sessionID.
function confirmHeader(p, sessionID) {
    let data;
    try {
