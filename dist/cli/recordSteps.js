 ...
 */
export function printable(data) {
    let out = "";
    for (const b of data) {
        if (b === 0x1b)
            out += "\\x1b";
 ...
 */
export function decodeKeys(spec) {
    const out = [];
    for (let i = 0; i < spec.length; i++) {
        ...
        if (i + 1 >= spec.length) {
            throw new Error(`trailing backslash in key spec: ${JSON.stringify(spec)}`);
        }
        const esc = spec[i + 1];
        if (esc === "x") {
            const digits = spec.slice(i + 2, i + 4);
            if (!hex.test(digits)) {
                throw new Error(`bad \\x escape in key spec: ${JSON.stringify(spec)} (expected two hex digits after \\x)`);
            }
        ...
        if (esc === "t")
            ...
            throw new Error(`unknown escape \\${esc} in key spec: ${JSON.stringify(spec)} (supported: \\xNN, \\t, \\r, \\n, \\\\)`);
        }
        i += 1;
    }
...
export function parseKeysSpec(spec) {
    ...
    for (const [i, burst] of bursts.entries()) {
        if (burst === "") {
            throw new Error(`empty key burst at position ${String(i + 1)} of ${JSON.stringify(spec)}; ` +
...
// --- validation --------------------------------------------------------------
function positiveInt(value, field, kind) {
    if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
        throw new Error(`${kind} step: ${field} must be a positive integer (got ${JSON.stringify(value)})`);
    }
}
function nonEmptyString(value, field, kind) {
    if (typeof value !== "string" || value === "") {
        throw new Error(`${kind} step: ${field} must be a non-empty string (got ${JSON.stringify(value)})`);
    }
}
/**
 ...
 */
export function validateDumpFile(file) {
    nonEmptyString(file, "file", "dump");
    const f = file;
    if (f.includes("/") || f.includes("\\")) {
        throw new Error(`dump step: file must be a bare filename, not a path (got ${JSON.stringify(f)})`);
    }
    if (f.includes("..")) {
        throw new Error(`dump step: file must not contain ".." (got ${JSON.stringify(f)})`);
    }
    if (f === "." || f.includes("\0")) {
...
export function validateStep(step) {
    ...
    if (typeof kind !== "string" ||
        ...
        throw new Error(`unknown step kind ${JSON.stringify(kind)} (expected one of: ${stepKinds.join(", ")})`);
    }
    switch (step.kind) {
        case "prompt":
        ...
        case "answer": {
            ...
            if (one === many) {
                throw new Error("answer step: exactly one of optionID or optionIDs must be set");
            }
            if (one)
                nonEmptyString(step.optionID, "optionID", "answer");
            ...
            if (ids !== undefined) {
                ...
                for (const id of ids)
                    nonEmptyString(id, "optionIDs entry", "answer");
            }
            break;
        }
        case "keys":
            ...
            if (step.label !== undefined)
                nonEmptyString(step.label, "label", "keys");
        ...
        case "settle":
            if (typeof step.ms !== "number" ||
                ...
                step.ms < 0) {
                throw new Error(`settle step: ms must be a non-negative integer (got ${JSON.stringify(step.ms)})`);
            }
            break;
        case "dump":
...
export function expandScenario(scenario) {
    ...
    if (prompts !== undefined && scripted !== undefined) {
        throw new Error("scenario declares both prompts and steps; use one (prompts is sugar for [prompt, await-turn] per entry)");
    }
    ...
    if (scenario.interrupt) {
        ...
        if (last < 0) {
            throw new Error("scenario sets interrupt: true but has no await-turn step to replace");
        }
...
//# sourceMappingURL=recordSteps.js.map