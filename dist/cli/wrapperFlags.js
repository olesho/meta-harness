...
const FLAG_DEFS = [
    {
        ...
        type: "string",
        def: "",
        usage: "path to write trace events as NDJSON (default: trace events are dropped)",
        set: (a, v) => {
            a.traceFile = v;
        },
    },
    {
        ...
        type: "bool",
        def: "false",
        usage: "write trace events as NDJSON to stderr (mutually exclusive with --trace-file)",
        set: (a, v) => {
    ...
    },
    {
        ...
        type: "string",
        def: "",
        usage: "reasoning effort for supported harnesses (low, medium, high, xhigh, max)",
        set: (a, v) => {
            a.effort = v;
        },
    },
    ...
    },
    {
        ...
        type: "string",
        def: "",
        usage: "spawn the run inside a detached tmux session named mh-<value> and exit immediately",
        set: (a, v) => {
            a.tmuxSession = v;
        },
    },
...
export function renderFlagSurface() {
    const lines = FLAG_DEFS.map((f) => `--${f.name} <${f.type}> = ${JSON.stringify(f.def)} : ${f.usage}`);
    lines.sort();
    return lines.join("\n") + "\n";
}
function emptyArgs() {
 ...
 */
export function parseHarnessWrapperArgs(argv) {
    const sep = argv.indexOf("--");
    if (sep === -1) {
        return {
            result: null,
            err: new Error("harness-wrapper: missing -- separator before harness args"),
        };
    }
    ...
    while (i < pre.length) {
        ...
        if (!def) {
            return {
                result: null,
                err: new Error(`harness-wrapper: flag provided but not defined: -${name}`),
            };
        }
        if (def.type === "bool") {
    ...
    if (args.traceFile !== "" && args.traceStderr) {
        return {
            result: null,
            err: new Error("harness-wrapper: --trace-file and --trace-stderr are mutually exclusive"),
        };
    }
    if (args.tmuxSession !== "" && args.tmuxChild !== "") {
        return {
            result: null,
            err: new Error("harness-wrapper: --tmux-session and --tmux-child are mutually exclusive"),
        };
    }
    ...
    if (positional.length !== 1) {
        return {
            result: null,
            err: new Error(`harness-wrapper: expected exactly one harness name before --, got ${String(positional.length)} args ` +
...
//# sourceMappingURL=wrapperFlags.js.map