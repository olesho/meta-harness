 ...
 */
async function waitReady(live, harness, timeoutMs) {
...
export async function main(argv) {
    ...
    const cwd = p.cwd || join(tmpdir(), "meta-harness-corpus-rec", p.harness, p.scenario);
    ...
    try {
        for (const [i, prompt] of scenario.prompts.entries()) {
            if (waitsForReady)
                await waitReady(live, p.harness, 90_000);
