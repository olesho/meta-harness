...
// stuck-looking one.
import { parseResetTime, parseRetryAfter, } from "../detector/detector.js";
...
const apiErrorRE = new RegExp(`^${horizontalSpace}*(?:${glyphs}${horizontalSpace}*)?API Error:${horizontalSpace}*(?:(\\d{3})\\b${horizontalSpace}+)?(.*)$`, "im");
/**
 ...
 * rejected.
 */
export function matchAPIError(stripped) {
    const m = apiErrorRE.exec(stripped);
    if (!m)
        return null;
    const message = (m[2] ?? "").trim();
    const hit = { code: 0, message, retryAfter: 0 };
    if (m[1]) {
...
const sessionLimitRE = new RegExp(`^${horizontalSpace}*(?:${glyphs}${horizontalSpace}*)?(You(?:'ve|\\s+have)\\s+hit\\s+your\\s+(?:session|usage)\\s+limit.*)$`, "im");
/** MatchSessionLimit implements a SessionLimitMatcher for Claude Code. */
export function matchSessionLimit(stripped, now) {
