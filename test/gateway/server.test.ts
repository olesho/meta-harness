...
import { Server, newToken, parseBind, type ConversationLike } from "../../src/gateway/server.ts"
...
function mkTurn(sessionID: string, role: Turn["role"], state: Turn["state"], text: string): Turn {
  const now = new Date()
  return {
...
class FakeConversation implements ConversationLike {
  ...
  async answer(ctx: Context, requestID: string, ans: InputAnswer): Promise<void> {
    if (this.closed || this.forceClosed) throw ErrClosed
    if (!this.queue.held()) throw ErrNoControl
    if (this.block) {
      await Promise.race([ctx.done(), this.closedSignal()])
  ...
  screenSnapshot(): Snapshot {
    return { text: "SCREEN", cols: 80, rows: 24, cursorCol: 3, cursorRow: 1, generation: 7 }
  }
...
function start(open?: (opts: unknown) => Promise<ConversationLike>): Promise<Live> {
  const gateway = new Server(open ? { open: open as never } : {})
...
async function open(base: string, fake: FakeConversation): Promise<string> {
  const r = await req(base, "POST", "/v1/conversations", { harness: "claude-code", binary_path: "/bin/x" })
...
describe("route-level integration flow", () => {
  test("open → control → send → SSE events → answer input → history → release → close", async () => {
    ...
    const snd = await req(base, "POST", `/v1/conversations/${id}/messages`, { token, text: "hi" })
    ...
    const rel = await req(base, "DELETE", `/v1/conversations/${id}/control/${token}`)
    expect(rel.status).toBe(204)
    // Releasing again → 404 unknown_token.
    const rel2 = await req(base, "DELETE", `/v1/conversations/${id}/control/${token}`)
...
describe("bind / trust boundary", () => {
  ...
  test("module header carries Go's trust-boundary note verbatim", () => {
    ...
    const src = readFileSync(join(here, "..", "..", "src", "gateway", "server.ts"), "utf8")
...
describe("id scheme", () => {
  test("open id === Conversation.sessionID(); history/screen/close address by it", async () => {
    ...
    const list = await req(base, "GET", "/v1/conversations")
    expect(list.json[0]).toMatchObject({ id: "the-session-id", session_id: "the-session-id" })

    expect((await req(base, "GET", `/v1/conversations/${id}/screen`)).status).toBe(200)
    expect((await req(base, "GET", `/v1/conversations/${id}/history`)).status).toBe(200)
    expect((await req(base, "DELETE", `/v1/conversations/${id}`)).status).toBe(204)
  })
  ...
  test("second open of the same session id is deduped/rejected", async () => {
    ...
    expect((await req(base, "POST", "/v1/conversations", { harness: "x", binary_path: "/b" })).status).toBe(201)
    const second = await req(base, "POST", "/v1/conversations", { harness: "x", binary_path: "/b" })
...
describe("close vs in-flight", () => {
  test("an op racing DELETE resolves to a live result or 410 gone, never half-torn-down", async () => {
    ...
    fake.block = true
    const sendP = req(base, "POST", `/v1/conversations/${id}/messages`, { token, text: "x", timeout_seconds: 5 })
    await delay(20)
  ...
  test("op on a closed-but-registered conversation → 410 gone", async () => {
    ...
    const snd = await req(base, "POST", `/v1/conversations/${id}/messages`, { token: ctl.json.token, text: "x" })
...
describe("error mapping via real requests", () => {
  test("send with no token → 409 no_control", async () => {
    ...
    const { base } = await serverFor(fake)
    const id = await open(base, fake)
    const r = await req(base, "POST", `/v1/conversations/${id}/messages`, { token: "nope", text: "x" })
  ...
  test("answer with no token → 409 no_control", async () => {
    ...
    const { base } = await serverFor(fake)
    const id = await open(base, fake)
    const r = await req(base, "POST", `/v1/conversations/${id}/input`, { token: "nope", option_id: "y" })
    expect(r.status).toBe(409)
    expect(r.json.code).toBe("no_control")
  })
  ...
  test("send with a turn in flight → 409 turn_in_flight", async () => {
    ...
    const { base } = await serverFor(fake)
    const id = await open(base, fake)
    const token = (await req(base, "POST", `/v1/conversations/${id}/control`)).json.token
    expect((await req(base, "POST", `/v1/conversations/${id}/messages`, { token, text: "a" })).status).toBe(202)
    const r = await req(base, "POST", `/v1/conversations/${id}/messages`, { token, text: "b" })
    expect(r.status).toBe(409)
  ...
  test("unknown harness at open → 400 unknown_harness", async () => {
    ...
    const r = await req(base, "POST", "/v1/conversations", { harness: "bogus", binary_path: "/b" })
  ...
  test("deadline → 504 timeout", async () => {
    ...
    const token = (await req(base, "POST", `/v1/conversations/${id}/control`)).json.token
    fake.block = true
    const r = await req(base, "POST", `/v1/conversations/${id}/messages`, { token, text: "x", timeout_seconds: 0.05 })
  ...
  test("cancel → 408 canceled", async () => {
    ...
    const r = await req(base, "POST", `/v1/conversations/${id}/messages`, { token, text: "x" })
  ...
  test("unknown / empty option_id → 400 unknown_option", async () => {
    ...
    const id = await open(base, fake)
    const token = (await req(base, "POST", `/v1/conversations/${id}/control`)).json.token
    fake.surface({ id: "ir", kind: "question", prompt: "?", options: [{ id: "yes", label: "Yes" }] })
    // Unknown id.
    const bad = await req(base, "POST", `/v1/conversations/${id}/input`, { token, option_id: "nope" })
    expect(bad.status).toBe(400)
    expect(bad.json.code).toBe("unknown_option")
    // Empty (no selection) against a menu prompt.
    const empty = await req(base, "POST", `/v1/conversations/${id}/input`, { token, option_id: "" })
...
describe("multi-select round trip", () => {
  ...
  test("rejection: multiple ids on a single-select prompt → 400 not_multi_select", async () => {
    ...
    const r = await req(base, "POST", `/v1/conversations/${id}/input`, { token, request_id: "ss", option_ids: ["a", "b"] })
    expect(r.status).toBe(400)
  ...
  test("rejection: unknown id in option_ids[] → 400 unknown_option", async () => {
    ...
    const r = await req(base, "POST", `/v1/conversations/${id}/input`, { token, request_id: "ms2", option_ids: ["a", "z"] })
    expect(r.status).toBe(400)
...
describe("no lost events", () => {
  test("turn/input events emitted before the first SSE subscriber are still delivered", async () => {
    ...
    fake.bus.emit({ type: "turn", turn: mkTurn(fake.id, "assistant", "complete", "early-1") })
    fake.bus.emit({ type: "input_request", input: { id: "early-ir", kind: "question", prompt: "?" } })
...
describe("/v1/turns (scaffold — pending A1's RunTurn/TurnResult)", () => {
  test("exit_after_turn=false → 400 unsupported", async () => {
    const { base } = await start()
    const r = await req(base, "POST", "/v1/turns", { harness: "x", binary_path: "/b", exit_after_turn: false })
    expect(r.status).toBe(400)
...
interface Collector {
  take(n: number, timeoutMs: number): Promise<Array<{ type: string; [k: string]: unknown }>>
  close(): void
}
