...
describe("Fanout", () => {
  test("multi-subscriber: two subscribers each receive every event", async () => {
    ...
    expect(ra.map((e) => e.turn?.id ?? e.input?.id)).toEqual([
      "t1",
      "i1",
      "t2",
    ]);
    expect(rb.map((e) => e.turn?.id ?? e.input?.id)).toEqual([
      "t1",
      "i1",
      "t2",
    ]);
  ...
  test("no-lost-events: events emitted before first subscribe are replayed", async () => {
    ...
    expect(got.map((e) => e.turn?.id ?? e.input?.id)).toEqual([
      "pre1",
      "pre2",
      "post1",
    ]);
