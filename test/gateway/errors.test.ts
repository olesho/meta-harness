...
import { ctxCanceled, ctxDeadlineExceeded, wrap } from "../../src/internal/async/index.ts"
import {
  mapChatError,
  mapRunTurnError,
  writeChatError,
...
describe("writeChatError — sentinel table", () => {
  ...
  test("unmatched error → 500 internal", () => {
    ...
    expect(mapChatError(new Error("boom"))).toEqual({ status: 500, code: "internal" })
  })
})

describe("writeRunTurnError — adds context sentinels", () => {
  test("ctxDeadlineExceeded → 504", () => {
    expect(mapRunTurnError(ctxDeadlineExceeded)).toEqual({ status: 504, code: "timeout" })
  ...
  test("ctxCanceled → 408", () => {
    expect(mapRunTurnError(ctxCanceled)).toEqual({ status: 408, code: "canceled" })
  ...
  test("still delegates chat sentinels to the chat table", () => {
    expect(mapRunTurnError(ErrTurnInFlight)).toEqual({ status: 409, code: "turn_in_flight" })
  })

  test("context sentinel through a cause chain resolves", () => {
    const got = capture(writeRunTurnError, wrap("run turn", ctxDeadlineExceeded))
    expect(got.status).toBe(504)
    expect(got.body.code).toBe("timeout")
  })
