...
import { checkAll, checkEntry, type Row, type Status } from "../../src/drift/sentry.ts"
...
describe.skipIf(!live)("registry-drift live conformance (META_HARNESS_CONFORMANCE=1)", () => {
  test(
  ...
  test(
    ...
    async () => {
      ...
      const row = await checkEntry("opencode", opencode!.package, opencode!.pinned)
