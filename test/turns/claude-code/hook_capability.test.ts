...
import type { Adapter, HookProviderCapability } from "../../../src/turns/types.ts"
import {
...
function probe(a: Adapter): HookProviderCapability | undefined {
  ...
  return typeof cap.hookProvider === "function" ? (cap as HookProviderCapability) : undefined
}
...
const coTenant: SettingsHookMatcher = {
  matcher: "*",
  hooks: [{ type: "command", command: "/opt/other-harness/wrap.sh # other-harness" }],
}

function managedMatchers(matchers: SettingsHookMatcher[]): SettingsHookMatcher[] {
  return matchers.filter((m) => m.hooks.some((h) => isManagedHookCommand(h.command)))
}
...
describe("Claude adapter ensureConfig — idempotency + co-tenancy", () => {
  ...
  test("preserves a co-tenant block across repeated ensureConfig", () => {
    ...
    writeFileSync(cfg, JSON.stringify({ hooks: { Stop: [coTenant] } }, null, 2))
  ...
  test("parsePayload delegates to the Claude payload parser", () => {
    ...
    const events = provider.parsePayload(raw, { ...ctxFor(dir), harnessSessionID: "s1" })
    expect(events.length).toBeGreaterThan(0)
    // A stray session is dropped by the mismatch guard.
    const dropped = provider.parsePayload(raw, { ...ctxFor(dir), harnessSessionID: "other" })
