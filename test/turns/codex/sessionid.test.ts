...
describe("codex session id", () => {
  ...
  test("resumeArgs leads with the `resume <uuid>` subcommand", () => {
    const args = codex.New().resumeArgs("sess-uuid", ["--full-auto"])
    expect(args).toEqual(["resume", "sess-uuid", "--full-auto"])
  })

  test("resumeArgs works with no base args", () => {
    expect(codex.New().resumeArgs("sess-uuid", [])).toEqual(["resume", "sess-uuid"])
  })
})
