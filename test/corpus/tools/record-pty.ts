...
async function main(): Promise<void> {
  ...
  const writeStdin = (data: Uint8Array, label: string) => {
    appendFileSync(
      stdinLogPath,
      `${stamp()}s  ${label.padEnd(10)} ${printable(data)}\n`,
    );
    pty.write(data);
  };
  ...
  const meta: Record<string, unknown> = {
    ...
    notes:
      o.notes ||
      `record-pty ${o.interactive ? "interactive" : "probe"} capture`,
  };
  ...
  if (o.interactive) {
    ...
    console.error(
      `[record-pty] interactive; Ctrl-] to detach. Recording to ${o.out}`,
    );
    return;
  }
