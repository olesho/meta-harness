...
function rolloutFiles(): Set<string> {
  ...
  const walk = (dir: string): void => {
    ...
    for (const e of entries) {
      ...
      else if (e.name.startsWith("rollout-") && e.name.endsWith(".jsonl")) out.add(e.name)
    }
  }
  walk(sessionsRoot)
  return out
...
async function main(): Promise<void> {
  ...
  if (!(await waitFor("ready composer", o.readyTimeoutMs, () =>
  ...
  let scrapedID = ""
  if (!(await waitFor("/status session id", 10_000, () => {
  ...
  if (!(await waitFor("ready composer after /status", 10_000, () =>
    readyForInput("codex", screen.snapshot().text),
  ...
  const swallowed = () => adapter.promptNotAccepted(screen.snapshot(), "\0never\0")
  ...
  let rolloutName = ""
  if (!(await waitFor("rollout file", o.replyTimeoutMs, () => {
  ...
  console.log(`[smoke] rollout appeared: ${rolloutName} (uuid matches /status id)`)

  // 5. CodexReader.read(scrapedId) must return the turn.
  let turns: { role: string; text: string }[] = []
  if (!(await waitFor("transcript turns", 30_000, () => {
    try {
  ...
  console.log(`[smoke] PASS — /status id == rollout uuid == readable transcript (${version})`)
