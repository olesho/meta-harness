...
async function scoreOne(sc: Scenario): Promise<Record> {
  const meta = JSON.parse(readFileSync(join(sc.path, "meta.json"), "utf8")) as Meta
...
function emitMarkdown(records: Record[], threshold: number): void {
  ...
  lines.push("")
  lines.push(`Corpus harness adapter: \`${HARNESS}\` | threshold: ${threshold}`)
  lines.push("")
  lines.push("| Scenario | Harness | Cols | Rows | Exact | Distance | NormDist |")
  lines.push("|---|---|---|---|---|---|---|")
