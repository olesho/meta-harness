...
const scenarios: Record<string, Scenario> = {
  ...
  "interrupted-mid-reply": {
    ...
    interrupt: true,
    notes:
      "long reply interrupted with ESC mid-stream; must end errored, not complete",
  },
};
...
async function main(): Promise<void> {
  ...
  const rawVersion = execFileSync(resolved, ["--version"], {
    encoding: "utf8",
  }).trim();
