...
describe("ensureSettingsJSONHooks", () => {
  test("creates the managed block on a missing file", () => {
    ...
    expect(isManagedHookCommand(settings.hooks.Stop[0].hooks[0].command)).toBe(true)
  ...
  test("is idempotent: repeated ensure yields exactly one managed block", () => {
    const dir = tempDir()
    const cfg = path.join(dir, "settings.json")
    for (let i = 0; i < 5; i++) ensureSettingsJSONHooks(cfg, managedFor("Stop"))
  ...
  test("preserves unrelated top-level settings keys", () => {
    const dir = tempDir()
    const cfg = path.join(dir, "settings.json")
    writeFileSync(cfg, JSON.stringify({ model: "opus", permissions: { x: 1 } }))
  ...
  test("many sequential ensures never corrupt JSON (single block)", () => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const managed = s.hooks.Stop.flatMap((m: SettingsHookMatcher) => m.hooks)
