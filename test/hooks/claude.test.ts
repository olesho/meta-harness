...
describe("Claude payload parsing → SourceHook events", () => {
  ...
  test("Stop → turn-boundary marker", () => {
    const out = parseClaudeHookPayload(
      { session_id: SID, hook_event_name: HookEventStop, stop_hook_active: true },
      ctx(),
    )
    expect(out).toHaveLength(1)
  ...
  test("PostToolUse with no response text → boundary only", () => {
    const out = parseClaudeHookPayload(
      { session_id: SID, hook_event_name: HookEventPostToolUse, tool_name: "Task" },
      ctx(),
    )
    expect(out).toHaveLength(1)
    expect(out[0].event.type).toBe(EventTurnBoundary)
  ...
  test("unrecognized hook event → dropped", () => {
    expect(
      parseClaudeHookPayload({ session_id: SID, hook_event_name: "PreToolUse" }, ctx()),
    ).toEqual([])
  })
})
...
describe("session-mismatch guard", () => {
  test("mismatched session id is dropped", () => {
    const out = parseClaudeHookPayload(
      { session_id: "deadbeef-0000-0000-0000-000000000000", hook_event_name: HookEventStop },
      ctx(),
    )
    expect(out).toEqual([])
  })
...
describe("path-traversal guard", () => {
  test("valid transcript path under the projects dir passes", () => {
    const out = parseClaudeHookPayload(
      { session_id: SID, hook_event_name: HookEventStop, cwd: CWD, transcript_path: okTranscript },
      ctx(),
    )
    expect(out).toHaveLength(1)
  })
  ...
  test("guardPath accepts an in-bounds path", () => {
    expect(guardPath("/base/dir", "child/file.txt")).toBe("/base/dir/child/file.txt")
...
describe("provider surface", () => {
  test("ensureConfig resolves a HookSpec at settings.json", () => {
    ...
    expect(spec.events.some((e) => e.event === HookEventSessionStart)).toBe(true)
  })
