...
describe("OpenShellContainment", () => {
  ...
  describe("layer", () => {
    ...
    describe("execWrap", () => {
      ...
      it("consumes opts.cwd into --workdir (no host cwd leak)", () => {
        const containment = openshell({ cli })
        const layer = containment.layer(SB)
        const [wrapped, rest] = layer.execWrap(["test"], { cwd: "/sandbox/repo/sub" })
        const i = wrapped.indexOf("--workdir")
      ...
      it("crosses env as an in-guest `env K=V` prefix and strips it from opts", () => {
        ...
        expect(wrapped.slice(sep + 1)).toEqual(["env", "FOO=bar baz", "printenv", "FOO"])
  ...
  describe("acquire", () => {
     ...
     *  per-argv results. */
    function recordingWorkspace(script?: (argv: string[]) => ExecResult | undefined) {
      const calls: Array<{ argv: string[]; opts?: ExecOpts }> = []
      const ws: Workspace = {
    ...
    it("passes --from when the containment was built with one", async () => {
      const { ws, calls } = recordingWorkspace()
      const containment = openshell({ cli, agentId: "acq-from", from: "node:22-slim" })
    ...
    it("throws when neither policy.agentId nor opts.agentId is set", async () => {
      const { ws, calls } = recordingWorkspace()
      const containment = openshell({ cli })
      await expect(containment.acquire!(ctx, ws, {})).rejects.toThrow(/agentId/)
      expect(calls).toEqual([]) // nothing acquired
    })

    it("throws with stderr when create fails", async () => {
      const { ws } = recordingWorkspace((argv) =>
        argv[2] === "create" ? { code: 1, stdout: "", stderr: "no such image" } : undefined,
      )
      const containment = openshell({ cli, agentId: "acq-fail" })
      await expect(containment.acquire!(ctx, ws, {})).rejects.toThrow(/no such image/)
    })

    it("best-effort deletes the sandbox when mkdir prep fails (no leak)", async () => {
      const { ws, calls } = recordingWorkspace((argv) =>
        argv.includes("mkdir") ? { code: 1, stdout: "", stderr: "mkdir denied" } : undefined,
      )
      const containment = openshell({ cli, agentId: "acq-mkdir" })
      await expect(containment.acquire!(ctx, ws, {})).rejects.toThrow(/mkdir denied/)
      const name = sandboxName("acq-mkdir")
      expect(calls[calls.length - 1]!.argv).toEqual(["openshell", "sandbox", "delete", name])
    })
    ...
    it("acquire-derived layer through compose(): double-destroy is a no-op", async () => {
      ...
      expect(deletes).toEqual([["openshell", "sandbox", "delete", sandboxName("acq-idem")]])
    })
