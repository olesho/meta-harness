...
async function waitTerminal(ctx: Context, conv: Conversation): Promise<Turn> {
  ...
  for (;;) {
    ...
    if (outcome === "cancel") throw ctx.err() ?? new Error("conformance: context done")
    const { value, ok } = outcome
    if (!ok) throw new Error("conformance: event channel closed before a terminal turn")
    const ev = value!
    if (
      ev.type === EventTurn &&
    ...
    test.skipIf(skip)(
      ...
      async () => {
        ...
        try {
          ...
          expect(turn.state, `${name}: turn errored: ${turn.reason ?? ""}`).toBe(
            TurnStateComplete,
          )
