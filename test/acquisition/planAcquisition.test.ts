...
function fakeAdapter(name: string, caps: FakeCaps): Adapter {
  ...
  if (caps.transcriptReader) a.readTranscript = (_id: string, _cwd: string) => []
  if (caps.interleaved !== undefined) a.streamInterleaved = () => caps.interleaved!
  return a
}
...
function profileFrom(
  adapter: Adapter,
  opts: { installed?: boolean; harness?: string; streamVersionSupported?: boolean } = {},
): ResolvedProfile {
...
function ctx(profile: ResolvedProfile, over: Partial<PlanContext> = {}): PlanContext {
...
describe("probeAdapter", () => {
  ...
  test("streamInterleaved marker is invoked, not just present", () => {
    ...
    expect(probeAdapter(fakeAdapter("x", { interleaved: false })).streamInterleaved).toBe(false)
  })
})
...
describe("resolveProfile", () => {
  ...
  test("streamJSONSupported is false when not installed, even if predicate is true", () => {
    const p = profileFrom(interleavedFake, { installed: false, streamVersionSupported: true })
    expect(p.streamJSONSupported).toBe(false)
  })

  test("falls back to pinned version when no detected version", () => {
    const p = resolveProfile({
      info: { harness: "codex", installed: true, detectedVersion: "", pinnedVersion: "0.142.5" },
...
describe("streamEligible", () => {
  test("true only when parser + version support + interleaved all hold", () => {
    expect(streamEligible(profileFrom(interleavedFake, { streamVersionSupported: true }))).toBe(true)
  })

  test("false when version predicate rejects (fact 3 gate)", () => {
    expect(streamEligible(profileFrom(interleavedFake, { streamVersionSupported: false }))).toBe(false)
  })

  test("false for the real A1 shape (not interleaved — condition c)", () => {
    expect(streamEligible(profileFrom(realShapeAdapter, { streamVersionSupported: true }))).toBe(false)
  })

  test("false when adapter has no StreamParser", () => {
    const noParser = fakeAdapter("noparser", { interleaved: true })
    expect(streamEligible(profileFrom(noParser, { streamVersionSupported: true }))).toBe(false)
  })
})
...
describe("planAcquisition", () => {
  test("no sink ⇒ Off regardless of mode", () => {
    const p = profileFrom(interleavedFake, { streamVersionSupported: true })
    for (const mode of [AcquisitionModeOff, AcquisitionModeStream, AcquisitionModeHooks] as const) {
      expect(planAcquisition(mode, ctx(p, { haveSink: false }))).toBe(AcquisitionModeOff)
    }
  })

  test("requested Off ⇒ Off", () => {
    const p = profileFrom(interleavedFake, { streamVersionSupported: true })
    expect(planAcquisition(AcquisitionModeOff, ctx(p))).toBe(AcquisitionModeOff)
  })

  describe("requested Stream", () => {
    test("Stream when the synthetic interleaved fake is eligible", () => {
      const p = profileFrom(interleavedFake, { streamVersionSupported: true })
      expect(planAcquisition(AcquisitionModeStream, ctx(p))).toBe(AcquisitionModeStream)
    })
    ...
    test("version-predicate gate: falls back to Hooks when version unsupported", () => {
      ...
      expect(planAcquisition(AcquisitionModeStream, ctx(p))).toBe(AcquisitionModeHooks)
    })
    ...
    test("falls back to Off when not eligible and hooks not viable", () => {
      const p = profileFrom(realShapeAdapter, { streamVersionSupported: true })
      expect(planAcquisition(AcquisitionModeStream, ctx(p, { hooksViable: false }))).toBe(
        AcquisitionModeOff,
      )
    })
  })
  ...
  describe("requested Hooks", () => {
    test("Hooks when hook delivery is viable", () => {
      const p = profileFrom(realShapeAdapter, { streamVersionSupported: true })
      expect(planAcquisition(AcquisitionModeHooks, ctx(p))).toBe(AcquisitionModeHooks)
    })

    test("degrades to Stream when hooks not viable but adapter is Stream-eligible", () => {
      const p = profileFrom(interleavedFake, { streamVersionSupported: true })
      expect(planAcquisition(AcquisitionModeHooks, ctx(p, { hooksViable: false }))).toBe(
        AcquisitionModeStream,
      )
    })
...
describe("replanAfterStreamFailure", () => {
  test("falls back to Hooks when hook delivery is viable", () => {
    const p = profileFrom(interleavedFake, { streamVersionSupported: true })
    expect(replanAfterStreamFailure(ctx(p, { hooksViable: true }))).toBe(AcquisitionModeHooks)
  })

  test("falls back to Off when hooks not viable", () => {
    const p = profileFrom(interleavedFake, { streamVersionSupported: true })
    expect(replanAfterStreamFailure(ctx(p, { hooksViable: false }))).toBe(AcquisitionModeOff)
  })
})
