...
const SOURCES: ("live" | "file")[] = ["live", "file"];
...
describe("admitParent decision table", () => {
  ...
  for (const kind of ALL_KINDS) {
    test(`stream: live parent admitted (kind=${kind})`, () => {
      expect(admitParent(AcquisitionModeStream, "live", kind, false)).toBe(
        true,
      );
    });
    test(`stream: file parent dropped (kind=${kind})`, () => {
      expect(admitParent(AcquisitionModeStream, "file", kind, false)).toBe(
        false,
      );
    });
  }
  ...
  for (const kind of CONVERSATION_KINDS) {
    test(`hooks: live parent conversation kind dropped (kind=${kind})`, () => {
      expect(admitParent(AcquisitionModeHooks, "live", kind, false)).toBe(
        false,
      );
    });
  }
  ...
  for (const source of SOURCES) {
    for (const kind of ALL_KINDS) {
      test(`off: parent dropped (source=${source} kind=${kind})`, () => {
        expect(admitParent(AcquisitionModeOff, source, kind, false)).toBe(
          false,
        );
      });
      test(`unknown mode: parent dropped (source=${source} kind=${kind})`, () => {
        expect(
          admitParent("bogus-mode" as AcquisitionMode, source, kind, false),
        ).toBe(false);
      });
    }
  }
