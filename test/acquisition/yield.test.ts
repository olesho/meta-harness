...
describe("YieldControl", () => {
  test("request → checkYield → clear round-trip", () => {
    ...
    const decision = JSON.parse(outcome.blockOutput) as { decision: string; reason: string }
  ...
  test("request writes atomically — no partial file, no stray temp files", () => {
    ...
    const parsed = JSON.parse(readFileSync(y.filePath(), "utf8")) as { reason: string }
