...
function mkSession(id: string): Session {
  return { id, harness: "codex", workingDir: "", createdAt: new Date(), harnessSessionID: "" }
}
...
describe("memstore", () => {
  ...
  test("duplicate createSession errors", async () => {
    ...
    await expect(s.createSession(mkSession("dup"))).rejects.toThrow(/already exists/)
  })
  ...
  test("updateSession missing errors", async () => {
    const s = newMemStore()
    await expect(s.updateSession(mkSession("ghost"))).rejects.toThrow(/not found/)
  })
  ...
  test("appendTurn for unknown session errors", async () => {
    const s = newMemStore()
    await expect(s.appendTurn(mkTurn("t", "ghost", "x"))).rejects.toThrow(/not found/)
  })
  ...
  test("updateTurn replaces in place; missing errors", async () => {
    ...
    await expect(s.updateTurn(mkTurn("ghost", "s", "x"))).rejects.toThrow(/not found/)
  })
