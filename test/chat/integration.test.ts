...
import { New, PromptRef, openFake, sendOneTurn, waitForTerminalTurn } from "./fakeharness.ts"
...
describe("chat integration (real pty + fake harness)", () => {
  ...
  test("MultiTurn recognizes independent turn boundaries", async () => {
    ...
    for (const _ of sentinels) {
      b = b.AwaitSubmit().Working(30, "Working").Reply(40, "Echo: " + PromptRef(), "Synthesized", "5s")
    }
  ...
  test("pi multi-turn", async () => {
    ...
    let b = New("pi").PiIdle()
    for (const _ of sentinels) {
      b = b.AwaitSubmitCR().PiWorking(20).PiReply(30, "echo: " + PromptRef())
    }
    const conv = await openTracked(b.Build())
