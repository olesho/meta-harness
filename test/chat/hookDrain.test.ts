...
import type { HookContext, HookProvider, HookSpec } from "../../src/hooks/provider.ts"
...
class StubProvider implements HookProvider {
  ...
  ensureConfig(ctx: HookContext): HookSpec {
    this.lastCtx = ctx
    return { configPath: path.join(ctx.configDir, "settings.json"), events: [], owner: "stub" }
  }
...
function hookEvent(id: string, text: string, type = "turn_boundary"): ParsedEvent {
  return {
    harnessSessionID: "s1",
...
describe("HookDrain — spool dir lifecycle", () => {
  test("ensureConfig installs the managed block and creates the per-session spool dir", () => {
    ...
    expect(drain.spoolDir()).toBe(path.join(configDir, hookSpoolSubdir, "sess-abc"))
  ...
  test("close runs a final flush drain to catch the tail, then reaps the spool dir", () => {
    ...
    expect(events.flat().map((pe) => pe.event.nativeID)).toContain("hook:tail:s1")
...
describe("HookDrain — independent drain wakeup", () => {
  test("bounded fallback timer drains a pre-existing spool with NO wake and NO fs event", async () => {
    ...
    expect(events.flat().map((pe) => pe.event.nativeID)).toContain("hook:idle:s1")
  })

  test("fs.watch on the spool dir raises the wake and drains promptly", async () => {
    const { drain, events } = newDrain({ fallbackMs: 5000 /* huge: only the watch can be prompt */ })
    drain.ensureConfig()
    drain.start()
    ...
    expect(events.flat().map((pe) => pe.event.nativeID)).toContain("hook:watched:s1")
  })
  ...
  test("an explicit wake signal drains without waiting for the timer", async () => {
    ...
    await delay(200)
    drain.close()
    expect(events.flat().map((pe) => pe.event.nativeID)).toContain("hook:sig:s1")
  })
  ...
  test("the loop terminates when the close promise resolves", async () => {
    ...
    let isClosed = false
    const { drain } = newDrain({ closed, isClosed: () => isClosed, fallbackMs: 5000 })
    drain.ensureConfig()
    drain.start()
    isClosed = true
    resolveClosed()
...
describe("HookDrain — routing & dedup (durable-store layer)", () => {
  test("routed events carry source === SourceHook", () => {
    ...
    const wire = JSON.parse(marshalParsedEvents(routed)) as Array<{ event: { source?: string } }>
  ...
  test("an event drained twice is routed only once (dedup across drains)", () => {
    ...
    appendSpool(drain.spoolDir(), [hookEvent("hook:dup:s1", "dup")])
    drain.drainOnce()
    expect(events.flat().filter((pe) => pe.event.nativeID === "hook:dup:s1")).toHaveLength(1)
  })
  ...
  test("dedup within a single batch collapses duplicate ids", () => {
    ...
    drain.drainOnce()
    expect(events.flat().filter((pe) => pe.event.nativeID === "hook:same:s1")).toHaveLength(1)
  })
  ...
  test("a SourceFile twin supplied via existing() supersedes the provisional hook event", () => {
    const fileTwin: ParsedEvent = {
      ...
      event: { role: "tool", type: "tool_use", toolUseID: "t1", source: "file", nativeID: "" },
    }
    const events: ParsedEvent[][] = []
    const configDir = tempDir()
    const drain = new HookDrain({
    ...
    appendSpool(drain.spoolDir(), [
      {
        harnessSessionID: "s1",
        event: { role: "tool", type: "tool_use", toolUseID: "t1", source: SourceHook },
      },
    ])
    drain.drainOnce()
...
describe("HookDrain — chat-surface projection is separate (no source)", () => {
  test("turn-boundary edges project to Turns that carry role/text but NO source", () => {
    ...
    drain.ensureConfig()
    appendSpool(drain.spoolDir(), [hookEvent("hook:stop:s1", "turn-end", "turn_boundary")])
    drain.drainOnce()
...
describe("HookDrain — end-to-end through the real hook subprocess", () => {
  test("hooks bin writes spool → drainOnce routes SourceHook events to the store layer", () => {
    ...
    const payload = JSON.stringify({ session_id: "sess-e2e", hook_event_name: "Stop" })
    ...
    const wire = JSON.parse(marshalParsedEvents(fresh)) as Array<{ event: { source?: string } }>
