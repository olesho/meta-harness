...
import { SourceHook, type ParsedEvent, type Turn as TranscriptTurn } from "../../src/transcript/event.ts"
...
function fireHook(spoolDir: string, sessionID: string, cwd: string, event: string): void {
...
describe("hook drain — integration through a Conversation (fake backend)", () => {
  ...
  test("a turn-boundary edge is projected SEPARATELY to a Turn with no observable source", async () => {
    ...
    fireHook(conv.hookDrain!.spoolDir(), conv.session.harnessSessionID, configDir, "Stop")
    ...
    for (const t of boundaryTurns) expect("source" in (t as object)).toBe(false)
  }, 30_000)
