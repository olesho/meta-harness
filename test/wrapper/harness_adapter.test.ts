...
const cases: Case[] = [
  {
    name: "A1: claude api_error 529 without idle gate",
    adapter: claude,
    input: { recentOutput: "API Error: 529 Overloaded." },
  ...
  },
  {
    ...
    adapter: claude,
    input: {
      recentOutput: "API Error: 429 Too Many Requests. Retry after 30 seconds.",
    },
    wantStatus: StatusAPIError,
    wantCode: 429,
    wantRetry: 30_000,
  ...
  },
  {
    ...
    adapter: codex,
    input: { recentOutput: "■ exceeded retry limit, last status: 503" },
    wantStatus: StatusAPIError,
    wantCode: 503,
  },
  {
    name: "A5: cost path on idle",
    adapter: claude,
    input: { recentOutput: "you've hit your limit", idle: true },
    wantStatus: StatusBlockedByCost,
  },
  {
    name: "A6: retry path on idle",
    adapter: claude,
    input: { recentOutput: "please try again", idle: true },
    wantStatus: StatusRetryLater,
  },
  {
    name: "A7: prompt detection on quiet trailing line",
    adapter: claude,
    input: { recentOutput: "Some text\nContinue? [y/N]", quiet: true },
  ...
  },
  {
    ...
    adapter: claude,
    input: {
      recentOutput: "you've hit your limit\nAPI Error: 529 Overloaded.",
      idle: true,
    },
    wantStatus: StatusAPIError,
    wantCode: 529,
  },
  {
    ...
    adapter: claude,
    input: {
      recentOutput: "chitchat about API Error: 500 mid-line",
      idle: true,
    },
    wantStatus: "",
  },
  {
    name: "A11: claude session-limit banner without idle gate",
    adapter: claude,
    input: {
      recentOutput:
...
describe("HarnessAdapter.classify", () => {
  for (const tc of cases) {
    test(tc.name, () => {
      ...
      const wantTerminal =
        tc.wantStatus === StatusBlockedByCost ||
        tc.wantStatus === StatusRetryLater;
