...
describe("classifyOutput assigns ErrorClass", () => {
  const cases: {
    ...
    {
      name: "api 402 → billing",
      harness: "claude",
      output: "API Error: 402 Payment Required",
      want: ErrBilling,
    },
    {
      name: "api 404 → model-not-found",
      harness: "claude",
    ...
    {
      name: "api 529 → transient",
      harness: "claude",
      output: "API Error: 529 Overloaded",
      want: ErrTransient,
    },
    {
      name: "transport → transient",
      harness: "claude",
      output: "Mock\nError: connection refused",
      want: ErrTransient,
    },
    {
      name: "usage limit → rate-limited",
      harness: "claude",
      output: "you've hit your usage limit",
      want: ErrRateLimited,
    },
    {
      name: "quota exceeded → billing",
      harness: "claude",
      output: "Error: quota exceeded",
      want: ErrBilling,
    },
    {
      name: "retry prose → transient",
      harness: "claude",
      output: "upstream error, please try again",
      want: ErrTransient,
    },
    {
      name: "benign → none",
...
describe("classifyOutput cursor pack", () => {
  const cases: {
    ...
    {
      name: "transient prose",
      output: "service unavailable, please try again",
      status: StatusRetryLater,
      class: ErrTransient,
    },
  ];
