...
async function awaitAPIError(
  ...
  for (;;) {
    ...
    if (recv === "timeout")
      throw new Error("timeout waiting for api_error event");
    if (!recv.ok)
      throw new Error("events channel closed before api_error arrived");
    const ev = recv.value!;
