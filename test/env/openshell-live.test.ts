...
describe.skipIf(!enabled)("openshell live (Tier-4)", () => {
  ...
  runConformance({
    ...
    makeProvisioner: () =>
      local({ root: mkdtempSync(join(tmpdir(), "os-live-")) }),
  ...
  test("env crossing: printenv sees the crossed value, cwd is the guest repo", async () => {
    ...
    try {
      const r = await ws.exec(ctx, ["printenv", "FOO"], {
        env: { FOO: "bar baz" },
      });
  ...
  test("raw CLI second delete of an already-gone sandbox (documents why teardown short-circuits)", async () => {
    ...
    console.warn(
      `openshell-live: redundant delete of ${name} exited ${second.code}: ` +
        stripAnsi(second.stderr || second.stdout)
          .trim()
          .slice(0, 200),
    );
  });
  ...
  afterAll(() => {
    ...
    if (ls.code !== 0) return;
    const leaked =
      stripAnsi(ls.stdout).match(/openshell-mh-live-[a-z0-9-]*/g) ?? [];
