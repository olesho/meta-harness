...
const live = process.env.META_HARNESS_ENV_LIVE === "daytona" && !!process.env.DAYTONA_API_KEY
...
describe.skipIf(!live)("Daytona provisioner (live)", () => {
  ...
  test(
    "sweep(): a labeled-but-not-destroyed sandbox is reaped and probes dead afterward",
    async () => {
      ...
      const ws = await prov.create(ctx, {
        image: "ubuntu:22.04",
        name: `sweep-${RUN_ID}`,
        labels: { "meta-harness-test": "1", "meta-harness-run": RUN_ID, "meta-harness-sweep-target": "1" },
        autoStopInterval: 5,
        autoDeleteInterval: 30,
      })
  ...
  test(
    "sweep(): dryRun reports the match without deleting",
    async () => {
      const prov = daytonaProvisioner(config)
      await prov.preflight(ctx)
      const ws = await prov.create(ctx, {
        ...
        labels: { "meta-harness-test": "1", "meta-harness-run": RUN_ID, "meta-harness-sweep-dry": "1" },
        autoStopInterval: 5,
        autoDeleteInterval: 30,
      })
      try {
...
afterAll(
  async () => {
    ...
    await daytonaSweep(ctx, { apiKey: process.env.DAYTONA_API_KEY }, {
