...
import { resetFakeDaytonaSdk, state as fakeSdkState } from "./fixtures/fake-daytona-sdk.ts"

const FAKE_SDK_URL = new URL("./fixtures/fake-daytona-sdk.ts", import.meta.url).href
...
describe("buildExecCommand / parseExecEnvelope (marker envelope)", () => {
  ...
  test("marker absent falls back to treating the whole payload as stdout", () => {
    ...
    expect(parseExecEnvelope(raw, "__MH_missing__")).toEqual({ stdout: raw, stderr: "" })
  })
...
describe("Daytona provisioner against a fake SDK", () => {
  ...
  test("upload(): single file creates the missing guest parent directory first", async () => {
    ...
    try {
      ...
      const mkdirCmd = fakeSdkState.executedCommands.find((c) => c.includes("mkdir"))
  ...
  test("destroy(): retention matrix on the delete spy + double-destroy idempotency", async () => {
    ...
    for (const outcome of ["success", "failure"] as const) {
      resetFakeDaytonaSdk()
      const ws = await prov.create(ctx, { image: "img", name: `absent-${outcome}` })
    ...
    resetFakeDaytonaSdk()
    {
      const ws = await prov.create(ctx, { image: "img", name: "keep-fail", retention: "keep-on-failure" })
  ...
  test("destroy(): keep-then-retry-with-different-outcome still keeps (flag-ordering regression)", async () => {
    ...
    fakeSdkState.execResult = () => ({ result: "", exitCode: 0 })
    const prov = daytona({ apiKey: "k", sdkImport: FAKE_SDK_URL })
    const ws = await prov.create(ctx, { image: "img", name: "order", retention: "keep-on-failure" })
...
describe("sweep()", () => {
  ...
  test("empty labels throw (billing-safety backstop)", async () => {
    await expect(sweep(ctx, { sdkImport: FAKE_SDK_URL }, { labels: {} })).rejects.toThrow(
      /empty labels/,
    )
  })
  ...
  test("filters client-side requiring every label pair to match", async () => {
    ...
    const result = await sweep(ctx, { sdkImport: FAKE_SDK_URL }, { labels: { run: "r1", tier: "test" } })
  ...
  test("drains the full (auto-paginating) list iterator before filtering", async () => {
    ...
    const result = await sweep(ctx, { sdkImport: FAKE_SDK_URL }, { labels: { run: "sweep-me" } })
  ...
  test("collects per-sandbox deletion failures without aborting the sweep", async () => {
    ...
    const result = await sweep(ctx, { sdkImport: FAKE_SDK_URL }, { labels: { run: "r1" } })
    expect(result.swept.sort()).toEqual(["ok-1", "ok-2"])
    expect(result.failed).toEqual([{ id: "boom", error: "fake delete failure for boom" }])
  })
})
