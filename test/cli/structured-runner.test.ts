...
describe("reportedPermissionRung (StructuredTurnResult.permission_mode)", () => {
  ...
  test("the unlock-only --allow-dangerously-skip-permissions is NOT reported as bypass", () => {
    ...
    // "override". (`default` is claude's hidden alias with no canonical rung;
    // this used to use dontAsk, which PUPPET-510 put on the ladder.)
    expect(
      rung("claude-code", {
        harnessArgs: [allow, "--permission-mode", "default"],
      }),
    ).toBe("default");
    // dontAsk is on the ladder (manual, PUPPET-510), and the flag leaves it there.
    expect(
      rung("claude-code", {
