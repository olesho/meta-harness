...
describe("parseStructuredArgs (safe-transport grammar)", () => {
  ...
  test("supports --flag=value form", () => {
    const p = parseStructuredArgs(["--prompt-file=/p", "--effort=low", "claude"])
  ...
  test("rejects a missing name, unknown flag, valueless flag, and bare trailing arg", () => {
    expect(parseStructuredArgs(["--prompt-file", "/p"]).error).toMatch(/missing <name>/)
    expect(parseStructuredArgs(["--bogus", "claude"]).error).toMatch(/unknown flag/)
    expect(parseStructuredArgs(["--prompt-file"]).error).toMatch(/requires a value/)
    expect(parseStructuredArgs(["claude", "extra"]).error).toMatch(/must follow/)
  })
...
describe("structured-runner main() — one-turn JSON contract (real pty + fake harness)", () => {
  ...
  const keys = ["HARNESS_BINARY_CLAUDE_CODE", EnvVar, "LOOM_WORKTREE_PATH", "LOOM_LOCAL_TASK_TIMEOUT_MS"]
  afterEach(() => {
    for (const k of keys) {
  ...
  async function captureMain(argv: string[]): Promise<{ code: number; payload: any }> {
    const chunks: string[] = []
  ...
  test("completed: emits one JSON line with reply + harnessSessionID; transcript read is best-effort", async () => {
    ...
    const script = New("claude-code")
      ...
      .Build()
    const scriptPath = join(mkdtempSync(join(tmpdir(), "sr-script-")), "script.json")
    writeFileSync(scriptPath, JSON.stringify(script), { mode: 0o600 })
    const promptPath = join(mkdtempSync(join(tmpdir(), "sr-prompt-")), "prompt.txt")
    ...
    const { code, payload } = await captureMain(["--prompt-file", promptPath, "claude"])
