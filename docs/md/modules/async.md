...
```ts
import { Context, ctxCanceled, ctxDeadlineExceeded, fromAbortSignal } from "meta-harness/async"
```
...
```ts
await ctx.done()
if (ctx.err() === ctxDeadlineExceeded) { /* timed out */ }
```
...
- If `deadlineMs` is a positive number, the context *also* auto-cancels with
...
`Context` is the *only* thing that crosses the boundary. The rest of the internal
