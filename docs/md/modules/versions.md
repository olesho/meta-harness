...
upstream release that adapter is _verified against_. The data is read at module load (the
...
`verifiedAt` documents _when_ the pin was confirmed and is only valid alongside a non-empty
...
`versions` is the static declaration of _what should be_; [`discovery`](discovery.md) does
the runtime check of _what is_ — resolving each `Entry`'s `binary` on `PATH`, probing its
