...
When you want a single _prompt → reply_ and nothing more, skip the
...
wedged harness. `Context.withDeadline` gives you a timer _and_ a `cancel()` you must call
in a `finally`.
...
try {
  ...
  if (err instanceof DeadlineError) {
    /* timed out */
  } else if (err instanceof TurnErroredError) {
...
When you need the outcome _and_ the harness session id even on failure — for example to
...
switch (outcome.status) {
  case "completed":
    ...
    break;
  case "errored":
    console.error("errored:", outcome.reason);
    break;
  case "deadline":
    console.error("timed out; session:", outcome.harnessSessionID);
    break;
  case "startup_error":
    console.error("never started:", outcome.reason);
    break;
}
```
