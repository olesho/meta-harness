...
2. Calls `ws.exec()` with `meta-harness-structured-run --prompt-file <path> … [--permission-mode P] [--sandbox-defaults] <harness> -- <args>` (`--sandbox-defaults` opts into `IS_SANDBOX=1` + the claude-code permission bypass; off by default — argv/env forwarded verbatim; `--permission-mode` composes with it rather than conflicting — when set it wins for argv, so no bypass is emitted, while `IS_SANDBOX=1` still applies unconditionally).
