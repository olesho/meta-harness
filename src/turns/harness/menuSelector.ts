 ...
 */
export function parseSelectorMenu(after: string): InputOption[] {
  ...
  for (let i = highlight - 1; i >= 0; i--) {
    if (!isSelectorSibling(lines[i], labelCol)) break;
    start = i;
  }
  ...
  if (rows < minSelectorRows || rows > maxSelectorRows) return [];

  const opts: InputOption[] = [];
  for (let i = start; i <= end; i++) {
    const r = runes(lineContent(lines[i]));
    if (labelCol >= r.length) return [];
 ...
 */
function selectorKeys(delta: number): Uint8Array {
  if (delta > 0) return enc.encode("\x1b[B".repeat(delta) + "\r");
  if (delta < 0) return enc.encode("\x1b[A".repeat(-delta) + "\r");
  return enc.encode("\r");
}
...
function isSelectorSibling(line: string, labelCol: number): boolean {
  if (isTerminator(line)) return false;
  const content = lineContent(line);
  if (content.includes(selectorGlyph)) return false;
...
function isTerminator(line: string): boolean {
  ...
  if (footerHintRE.test(trimmed)) return true;
  for (const ch of trimmed) {
    if (ch !== " " && !borderRunes.includes(ch)) return false;
  }
  return true;
}
...
function selectorLabelColumn(content: string): number {
  const r = runes(content);
  for (let i = 0; i < r.length; i++) {
    if (r[i] !== selectorGlyph) continue;
    let j = i + 1;
    while (j < r.length && (r[j] === " " || r[j] === "\t")) j++;
    if (j >= r.length) return -1;
    return j;
  }
  return -1;
}
...
// a blank line.
function textColumn(content: string): number {
  const r = runes(content);
  for (let i = 0; i < r.length; i++) {
    if (r[i] !== " " && r[i] !== "\t") return i;
  }
 ...
 * loud).
 */
export function hasChoiceShapedLine(after: string): boolean {
 ...
 */
export function candidateLines(after: string): string[] {
  const out: string[] = [];
  for (const ln of after.split("\n")) {
    const t = ln.trim();
    if (t === "") continue;
    out.push(t);
    if (out.length === maxSelectorRows) break;
  }
  return out;
}
...
export function aliasForLabel(label: string): string {
