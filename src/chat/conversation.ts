...
import { apiErrorVerdictFrom, turnReason } from "./apierror.ts";
import {
  ErrInvalidOptions,
  ErrUnknownHarness,
  ErrNoControl,
...
export class Conversation {
  ...
  private bypassEnabledAtLaunch(): boolean {
    return bypassReachableAtLaunch(
      this.opts.harness,
      this.opts.args ?? [],
  ...
  private relabelTerminal(turn: Turn, snap: Snapshot): boolean {
    return (
      this.apiErrorRelabel(turn) ||
  ...
  private apiErrorRelabel(turn: Turn): boolean {
    if (!this.hasTranscriptReader()) return false;
    const sessionID = this.session.harnessSessionID;
    if (sessionID === "" || this.sentTranscriptWatermark === null) return false;
    let turns: { role: string; text: string; apiError?: string }[];
    try {
    ...
    if (v.code !== undefined) turn.code = v.code;
    else delete turn.code;
  ...
  private hasTranscriptReader(): boolean {
  ...
  private readTranscriptTurns(
    id: string,
  ): { role: string; text: string; apiError?: string }[] {
    const a = this.adapter as unknown as Record<string, unknown>;
    return (
      a.readTranscript as (
        id: string,
  ...
  private captureTranscriptWatermark(): number | null {
    if (!this.hasTranscriptReader()) return null;
