...
import type { EnvConfig, Environment, Outcome, Redactor, Workspace } from "./types.ts"
 ...
 *  nowhere. */
const noopRedactor: Redactor = { register() {} }

export async function env(ctx: Context, cfg: EnvConfig): Promise<Environment> {
  ...
  let teardownWs: Workspace | undefined
  ...
  try {
    ...
    for (const inj of injectors) {
      for (const secret of inj.redactions()) redactor.register(secret)
    ...
    return {
      workspace: composed,
      async destroy(dctx: Context, outcome: Outcome = "success"): Promise<void> {
        ...
        if (errs.length > 0) throw new TeardownError(errs, "env.destroy")
      },
    }
  } catch (setupErr) {
