...
export type DaytonaClientCtor = new (
...
export async function loadDaytonaClass(
  config: DaytonaConfig,
): Promise<DaytonaClientCtor> {
...
export function buildExecCommand(
  ...
  const stdinPrefix =
    stdin !== undefined ? `printf %s ${shQuote(stdin)} | ` : "";
  return (
...
export function parseExecEnvelope(
  raw: string,
  marker: string,
...
class DaytonaWorkspace implements Workspace {
  ...
  private async execChecked(ctx: Context, argv: string[]): Promise<void> {
    ...
    if (r.code !== 0) {
      throw new Error(
        `daytona: ${argv.join(" ")} failed (code ${r.code}): ${r.stderr || r.stdout}`,
      );
    }
  }
