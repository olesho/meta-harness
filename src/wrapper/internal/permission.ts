 ...
 * guessed.
 */
export function isSupportedPermissionMode(
  harness: string,
  mode: string,
): boolean {
 ...
 */
export function harnessSupportsPermissionMode(harness: string): boolean {
  switch (normHarness(harness)) {
    case "claude":
    case "claude-code":
    case "codex":
 ...
 */
export function argsWithHarnessPermissionMode(
  harness: string,
  args: string[],
  mode: string,
): string[] {
  ...
  switch (normHarness(harness)) {
    case "claude":
    case "claude-code": {
      if (argsContainAnyFlag(args, claudeGuardFlags)) return args;
      const value = claudePermissionValue(mode);
      if (value === "") return args;
      return prependArgs(args, "--permission-mode", value);
    }
    case "codex": {
      if (argsContainAnyFlag(args, codexGuardFlags)) return args;
      if (argsContainConfigKey(args, codexSandboxConfigKey)) return args;
      if (argsContainConfigKey(args, codexApprovalConfigKey)) return args;
...
export type ArgvPermissionPin =
  { kind: "none" } | { kind: "native"; value: string } | { kind: "opaque" };
 ...
 */
export function argvPermissionPin(
  harness: string,
  args: string[],
): ArgvPermissionPin {
  switch (normHarness(harness)) {
    case "claude":
    case "claude-code": {
      if (!argsContainAnyFlag(args, claudeGuardFlags)) return { kind: "none" };
      ...
      if (!ok || value === "") return { kind: "opaque" };
      return { kind: "native", value };
    }
    case "codex": {
      if (
        argsContainAnyFlag(args, codexGuardFlags) ||
 ...
 * `bypassPermissions`.
 */
function claudePermissionValue(mode: string): string {
  switch (mode) {
 ...
 */
function codexPermissionArgs(mode: string): string[] {
  switch (mode) {
    case PermissionModePlan:
