...
export class EventChannel {
  ...
  private waiters: Array<(r: EventRecv) => void> = []
...
export class Session {
  private readonly cfg: Required<Pick<Config, "idleQuiet" | "idleClassify" | "staleThreshold" | "waitDelay">> &
    Config
  ...
  async wait(): Promise<{ result: Result; err: Error | null }> {
    ...
    return { result: this._result as Result, err: null }
  }
   ...
   * Idempotent.
   */
  async stop(ctx?: { done(): Promise<void>; err?: () => unknown }): Promise<Error | null> {
    ...
    const ctxErr = await Promise.race([
      ...
      ctx.done().then(() => (ctx.err?.() ?? new Error("context cancelled")) as Error),
    ])
    return ctxErr
  }
  ...
  private recordStatusChange(c: InternalClassification, terminated: boolean): void {
