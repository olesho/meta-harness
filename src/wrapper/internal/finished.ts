...
import { classifyOutput } from "./classifier.ts";
import { noClassification, type Classification } from "./classification.ts";
import {
  ErrAuth,
  ErrBilling,
  ErrContextOverflow,
...
const finishedTimeoutRe = new RegExp(
  `\\btimeout\\b|etimedout|connection${ANY}?timed?${ANY}?out|timed?${ANY}?out|deadline${ANY}?exceeded`,
  "i",
);
 ...
 */
const apiKeyVarUnset =
  `(?:ANTHROPIC|OPENAI|GEMINI|GOOGLE|CURSOR)_API_KEY[^\\n]{0,40}?(?:not set|unset|missing|required|invalid|empty)` +
  `|(?:not set|unset|missing|required|invalid|empty)[^\\n]{0,40}?(?:ANTHROPIC|OPENAI|GEMINI|GOOGLE|CURSOR)_API_KEY`;

interface ResidualRow {
  /**
   * A STABLE CONTRACT, not a description: loom's ADR 0002 names
   * `residual.auth` in a revisit trigger. Add rows freely; never rename one.
   */
  id: string;
  re: RegExp;
  cls: ErrorClass;
 ...
 */
const residualRows: readonly ResidualRow[] = [
  row(
    "residual.ratelimit",
    `\\b429\\b|too many requests|tokens per min|overloaded_error|resource${ANY}?exhausted|resource_exhausted|rate${ANY}?limit|usage${ANY}?limit|session${ANY}?limit|resets at|resets \\d{1,2}:\\d{2}|try again at${WS}+\\d`,
    ErrRateLimited,
    "rate limit exceeded",
  ),
  row(
    "residual.auth",
    `\\b401\\b|unauthorized|unauthenticated|forbidden|invalid${ANY}?api${ANY}?key|incorrect${ANY}?api${ANY}?key|authentication${ANY}?failed|` +
      apiKeyVarUnset,
    ErrAuth,
  ...
  row(
    "residual.billing",
    `\\b402\\b|payment${ANY}?required|insufficient${ANY}?(?:credits|quota)|insufficient_quota|exceeded${ANY}*quota|quota${ANY}?exceeded`,
    ErrBilling,
    "billing error",
  ),
  row(
    "residual.model_version",
    `model requires a newer version|requires a newer version of (?:codex|claude)|upgrade to the latest (?:app or )?cli`,
    ErrModelNotFound,
    "backend CLI is incompatible with the selected model",
  ),
  row(
    "residual.model_not_found",
    `model${ANY}?not${ANY}?found|model${ANY}*not found|model${ANY}*does not exist|model${ANY}*not${ANY}*exist|model_not_found|unsupported${ANY}?model|unknown${ANY}?model|invalid${ANY}?model|selected model${ANY}*may not exist|selected model${ANY}*may not have access to it|\\b404\\b${ANY}*model`,
    ErrModelNotFound,
    "model not found",
  ),
  row(
    "residual.context",
    `context${ANY}?length|context${ANY}?window|context_length_exceeded|maximum context length|max${ANY}?tokens|max${ANY}*tokens|token${ANY}?limit|prompt${ANY}?too${ANY}?long`,
    ErrContextOverflow,
  ...
  row(
    "residual.transient",
    `\\b50[023]\\b|\\b529\\b|server${ANY}?error|server_error|internal${ANY}?server${ANY}?error|internal${ANY}?error|service${ANY}?unavailable|backend${ANY}?error|overloaded`,
    ErrTransient,
    "server error",
  ),
];
 ...
 */
export function classifyFinishedOutput(
  harness: string,
  output: string,
): Classification {
  const c = classifyOutput(harness, output);
  if (c.status === StatusBinaryNotFound) return c;
  switch (c.class) {
    case ErrNone:
    case ErrUnknown: {
 ...
 */
function matchResidual(output: string): Classification | null {
 ...
 */
function firstUsableMatch(re: RegExp, s: string): string | null {
  for (const m of s.matchAll(re)) {
    const lo = m.index;
    const hi = lo + m[0].length;
    if (!embeddedNumber(s, lo, hi)) return m[0];
  }
  return null;
}
 ...
 */
function embeddedNumber(s: string, lo: number, hi: number): boolean {
  for (let i = lo; i < hi; i++) {
    if (!isDigit(s.charCodeAt(i))) return false; // a word row: unaffected
  }
  if (lo > 0 && s[lo - 1] === ".") return true;
  if (hi + 1 < s.length && s[hi] === "." && isDigit(s.charCodeAt(hi + 1)))
    return true;
  return false;
}

const retryAfterRe = new RegExp(
  `retry${ANY}?after[:\\t\\n\\f\\r ]+(\\d+)`,
  "i",
);
