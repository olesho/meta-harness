...
import { answerKeys } from "../chat/answerKeys.ts";
import { ErrUnknownOption } from "../chat/errors.ts";
...
import { isSentinel, wrap } from "../internal/async/index.ts";
import { Screen } from "../screen/index.ts";
import { DetectInput as claudecodeDetectInput } from "../turns/harness/claudecode.ts";
import {
  Errored,
  InputRequested,
  InputResolved,
...
// --- dialog spec (per-harness; claude-code only) ----------------------------
//
// The exact `interruptSpecs` precedent: a scenario whose TERMINAL STATE is a
// blocking startup dialog rather than a TurnComplete is recordable for a harness
// ONLY IF dialogSpecs has an entry for it. The folder-trust dialog is a
// claude-code concept — codex and pi have no equivalent startup dialog — so this
// map is the seam a future harness would extend rather than a flag to flip.
...
export const dialogSpecs: Record<string, DialogSpec> = {
...
// live session.
const questionSinglePrompt =
  "Use the AskUserQuestion tool to ask me exactly one question. The question " +
...
const questionMultiPrompt =
  "Use the AskUserQuestion tool to ask me exactly one MULTI-SELECT question " +
...
const questionReviewPrompt =
  "Use the AskUserQuestion tool once to ask me exactly TWO questions in a " +
...
//
// A catalog entry declares EITHER `prompts` (sugar: each entry desugars to a
// `prompt` + `await-turn` pair) OR an explicit `steps` script — never both.
// `expandScenario` turns whichever was declared into the one step list the
// interpreter runs, and that expanded list is what meta.json records, so the
// artifact says exactly what was driven.
interface Scenario {
...
export const scenarios: Record<string, Scenario> = {
  ...
  "trust-dialog": {
    ...
    notes:
      "first launch in an untrusted git repo; settles on the folder-trust " +
  ...
  "question-single": {
    ...
    notes:
      "AskUserQuestion SINGLE-SELECT question pane, captured UNANSWERED. " +
  ...
  "question-multi": {
    ...
    notes:
      "AskUserQuestion MULTI-SELECT (checkbox) pane. expected.txt is the " +
  ...
  "question-review": {
    ...
    notes:
      "AskUserQuestion REVIEW pane, reached by asking TWO questions in one " +
  ...
  "permission-mode-manual": {
    ...
    notes:
      "Permission-mode footer on the MANUAL rung — the load-bearing one of the " +
  ...
  "permission-mode-accept-edits": {
    ...
    notes:
      "Permission-mode footer on the ACCEPT EDITS rung: two Shift+Tabs from " +
  ...
  "permission-mode-plan": {
    ...
    notes:
      "Permission-mode footer on the PLAN rung: three Shift+Tabs from the " +
  ...
  "permission-mode-cycle": {
    ...
    notes:
      "The RING-LENGTH capture from a NORMAL launch (META-HARNESS-114): six " +
  ...
  "permission-mode-cycle-bypass": {
    ...
    notes:
      "The ring-length corroboration half (META-HARNESS-114): the same press " +
  ...
  "permission-mode-bypass": {
    ...
    notes:
      "Permission-mode footer on the BYPASS PERMISSIONS rung. Unlike the other " +
...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  let cwdFlagValue: string | undefined;
  let workdirFlagValue: string | undefined;
 ...
 */
export function claudeTrustState(
  dir: string,
  configPath: string,
): boolean | null {
  let cfg: unknown;
  ...
  const projects = (cfg as { projects?: Record<string, unknown> } | null)
    ?.projects;
  ...
  for (const key of candidates) {
    const entry = projects[key] as
      { hasTrustDialogAccepted?: unknown } | undefined;
 ...
 */
function adapterPermissionCycleKeys(
...
//
// One adapter instance, one polling cadence, one latch.
//
// ClaudeCodeAdapter.onScreen is EDGE-TRIGGERED: it fires InputRequested only
// when the detected request's `id` CHANGES, and InputResolved when the dialog
// disappears or is superseded. A second adapter, or a second polling loop,
// would each see a different slice of those edges and the one that missed the
// edge would wait forever. So every `await-*` step polls through the single
// `pump()` below, which drains the adapter once per 150 ms tick (the `waitFor`
// cadence) and LATCHES the current request into `ctx.lastInput` — `answer`
// reads the latch, never a live re-detection.

interface StepCtx {
  adapter: ReturnType<typeof resolveAdapter>;
  harness: string;
  /** The harness's submit key (Enter), resolved once. */
  submit: Uint8Array;
  /** Whether this harness gates prompts on a ready composer. */
  waitsForReady: boolean;
  ispec?: InterruptSpec;
  dspec?: DialogSpec;
  /** The adapter's permission-mode cycle keystroke, or undefined. */
  cycleKeys?: Uint8Array;
  out: string;
  stdinLog: string;
  /** Elapsed-seconds stamp for stdin.log, in record-pty.ts's format. */
  stamp: () => string;
  /** printable() rendering of every write, in order — recorded in meta.json. */
  keystrokes: string[];
  /** The pending input request, latched from the last InputRequested. */
  lastInput: InputRequest | null;
  /** Number of `cycle` presses so far, for screen-press-NN.txt numbering. */
  presses: number;
}
...
function pump(live: Live, ctx: StepCtx): Event[] {
  const evs = ctx.adapter.onScreen(live.screen.snapshot());
  for (const ev of evs) {
    if (ev.kind === InputRequested) ctx.lastInput = ev.input ?? null;
    else if (ev.kind === InputResolved) ctx.lastInput = null;
  }
  return evs;
}
...
function writeKeys(
  ...
  appendFileSync(
    ctx.stdinLog,
    `${ctx.stamp()}s  ${label.padEnd(10)} ${rendered}\n`,
  );
 ...
 */
const trustUnparseable =
  "folder-trust dialog is on screen but the adapter cannot parse its " +
...
async function runSteps(
  ...
  for (const [i, step] of steps.entries()) {
    ...
    switch (step.kind) {
      case "prompt": {
        if (ctx.waitsForReady) await waitReady(live, ctx.harness, 90_000, true);
        ...
        if (
          ...
          throw new Error(
            `prompt was not echoed into the composer: ${step.text}` +
              (probe === step.text
                ? ""
      ...
      case "interrupt": {
        ...
        if (!ispec) {
          throw new Error(
            `no interrupt spec for harness "${ctx.harness}" (gate missed)`,
          );
        }
      ...
      case "await-dialog-anchor": {
        ...
        if (!dspec) {
          throw new Error(
            `no dialog spec for harness "${ctx.harness}" (gate missed)`,
          );
        }
        ...
        await waitFor(
          live,
          dspec.what,
          (t) => dspec.anchors.some((a) => t.includes(a)),
          step.timeoutMs ?? 90_000,
        );
      ...
      case "await-input": {
        ...
        if (pending && (!want || pending.kind === want)) break;
        try {
          await waitFor(
            live,
            what,
      ...
      case "answer": {
        ...
        if (!req) {
          throw new Error(
            "no pending input request; an `answer` step must follow an " +
        ...
        try {
          ...
          if (isSentinel(err, ErrUnknownOption)) {
            ...
            throw wrap(
              `answer step: no option ${JSON.stringify(asked.join(", "))} on ` +
        ...
        await waitFor(
          live,
          "the answered dialog to resolve or be superseded",
          () => {
            const evs = pump(live, ctx);
            return evs.some(
              (ev) =>
                ev.kind === InputResolved ||
      ...
      case "keys": {
        ...
        process.stderr.write(
          `[screenbench-record] ${where}: ${printable(bytes)}\n`,
        );
      ...
      case "cycle": {
        ...
        if (!keys) {
          throw new Error(
            `harness "${ctx.harness}" has no permission-mode cycle keystroke (gate missed)`,
          );
        }
        ...
        for (let k = 0; k < n; k++) {
          ...
          writeFileSync(
            join(
              ctx.out,
              `screen-press-${String(ctx.presses).padStart(2, "0")}.txt`,
            ),
            live.screen.snapshot().text,
          );
        }
        break;
      }
      ...
      case "dump":
        ...
        // without the check.
        writeFileSync(
          join(ctx.out, validateDumpFile(step.file)),
          live.screen.snapshot().text,
        );
        break;
    }
 ...
 */
export function recorderOwnedMeta(meta: unknown): boolean {
  if (meta === null || typeof meta !== "object" || Array.isArray(meta))
    return false;
  const m = meta as Record<string, unknown>;
  const rec = m.recorder;
  if (typeof rec === "string") return rec.startsWith(recorderTag);
...
export async function main(argv: string[]): Promise<number> {
  ...
  if (catalogEntry && p.prompts.length > 0) {
    process.stderr.write(
      `screenbench-record: --prompt is for an ad-hoc script only, but ` +
        `"${p.scenario}" is a catalog scenario with its own prompts\n${USAGE}\n`,
    );
    return ExitUsage;
  }
  const scenario: Scenario = catalogEntry ?? {
    prompts: p.prompts,
  ...
  try {
    ...
    if (p.stopOnInput) {
      ...
      if (last >= 0 && steps[last].kind === "await-turn") steps[last] = stop;
      else steps.push(stop);
    }
    for (const spec of p.keys) steps.push(...parseKeysSpec(spec));
    if (steps.length === 0) {
    ...
    process.stderr.write(
      `screenbench-record: scenario "${p.scenario}": ` +
        (err instanceof Error ? err.message : String(err)) +
        `\n`,
    );
    return ExitUsage;
  ...
  if (scenario.requiresHarness && scenario.requiresHarness !== p.harness) {
    process.stderr.write(
      `screenbench-record: scenario "${p.scenario}" requires harness ` +
  ...
  if (
    steps.some((s) => s.kind === "await-dialog-anchor") &&
    !dialogSpecs[p.harness]
  ) {
    process.stderr.write(
      `screenbench-record: no dialog spec for harness "${p.harness}" — ` +
  ...
  let adapter: ReturnType<typeof resolveAdapter>;
  try {
  ...
  if (steps.some((s) => s.kind === "cycle") && !cycleKeys) {
    process.stderr.write(
      `screenbench-record: harness "${p.harness}" has no permission-mode cycle ` +
  ...
  if (scenario.freshWorkdir && p.cwd) {
    const configPath =
      process.env.META_HARNESS_CLAUDE_CONFIG ?? join(homedir(), ".claude.json");
    const trusted = claudeTrustState(p.cwd, configPath);
    if (trusted === true) {
      process.stderr.write(
        `screenbench-record: --cwd ${p.cwd} is already trusted by claude ` +
    ...
    if (trusted === null) {
      process.stderr.write(
        `[screenbench-record] warning: cannot read ${configPath} — proceeding ` +
      ...
      const mode =
        typeof m.mode === "string"
          ...
          : typeof m.recorder === "string"
            ...
            : "unset";
      process.stderr.write(
        `screenbench-record: ${p.out} holds a hand-captured recording (meta.json mode\n` +
  ...
  try {
    ...
    process.stderr.write(
      `screenbench-record: setup for scenario "${p.scenario}" failed in ` +
        `${cwd}: ` +
        (err instanceof Error ? err.message : String(err)) +
        "\n",
    );
  ...
  const launchArgs =
    p.launchArgs.length > 0 ? p.launchArgs : (scenario.launchArgs ?? []);
  ...
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      ...
      if (!endsOnDialog) await sleep(1_500); // settle so the final turn renders
    } catch (err) {
      ...
      if (err instanceof AwaitInputTimeout && attempt < p.attempts) {
        process.stderr.write(
          `[screenbench-record] attempt ${attempt}/${p.attempts} timed out ` +
  ...
  const meta = {
    ...
    keystrokes:
      endsOnDialog && keystrokes.length === 0
