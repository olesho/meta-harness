...
export const SUPPORTED_HARNESSES: readonly string[] = [
  "claude",
  "codex",
  "opencode",
  "pi",
 ...
 */
export function assertSupportedHarness(name: string): {
  ...
  if (!SUPPORTED_HARNESSES.includes(name)) {
    return {
      result: null,
      err: new Error(
        `unsupported harness ${JSON.stringify(name)} (supported: ${SUPPORTED_HARNESSES.join(", ")})`,
      ),
    };
  }
