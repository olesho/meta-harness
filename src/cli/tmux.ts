...
export function requireTmux(): Error | null {
...
export function resolveTracePath(
  explicit: string,
  sessionName: string,
): { result: string | null; err: Error | null } {
  if (explicit !== "") {
 ...
 */
export function runTmuxSpawn(args: HarnessWrapperArgs): number {
  if (!validSessionName(args.tmuxSession)) {
    process.stderr.write(
      `harness-wrapper: invalid --tmux-session value ${JSON.stringify(args.tmuxSession)} (allowed: [A-Za-z0-9_-], 1-64 chars)\n`,
    );
    return 2;
  }
  ...
  const { result: tracePath, err: traceErr } = resolveTracePath(
    args.traceFile,
    args.tmuxSession,
  );
  if (traceErr || !tracePath) {
    process.stderr.write(
      "harness-wrapper: " + (traceErr?.message ?? "resolve trace path") + "\n",
    );
    return 1;
  }
  try {
  ...
  const spawnResult = spawnSync(
    "tmux",
    ["new-session", "-d", "-s", tmuxName, ...reexec],
    {
  ...
  if (spawnResult.error || spawnResult.status !== 0) {
    process.stderr.write(
      `harness-wrapper: tmux new-session failed: ${spawnResult.error?.message ?? "exit " + String(spawnResult.status)}\n`,
    );
    return 1;
  }
...
function requireOneSessionArg(
  ...
  if (!validSessionName(args[0])) {
    process.stderr.write(
      `harness-wrapper: invalid session name ${JSON.stringify(args[0])}\n`,
    );
    return { result: "", code: 2 };
  }
 ...
 */
export function runTmuxAttach(argv: string[]): number {
  ...
  if (result.error) {
    process.stderr.write(
      `harness-wrapper: exec tmux attach: ${result.error.message}\n`,
    );
    return 1;
  }
...
export function runTmuxKill(argv: string[]): number {
  ...
  if (result.error || result.status !== 0) {
    process.stderr.write(
      `harness-wrapper: tmux kill-session failed: ${result.error?.message ?? "exit " + String(result.status)}\n`,
    );
    return 1;
  }
  return 0;
...
function listSessions(): string[] | null {
...
export function runTmuxList(argv: string[]): number {
  if (argv.length !== 0) {
 ...
 * the var.
 */
function lookupTraceFile(name: string): {
  result: string | null;
  err: Error | null;
} {
  const tmuxName = TMUX_SESSION_PREFIX + name;
...
function readLastTraceEvent(path: string): {
  ...
  if (lastLine === "") return { result: null, err: null };
  try {
    return {
...
export function runTmuxStatus(argv: string[]): number {
  let wantJSON = false;
  ...
  if (lookupErr || !tracePath) {
    process.stderr.write(
      "harness-wrapper: " + (lookupErr?.message ?? "lookup trace file") + "\n",
    );
    return 1;
  }
  ...
  if (readErr) {
    process.stderr.write(
      `harness-wrapper: read trace ${JSON.stringify(tracePath)}: ${readErr.message}\n`,
    );
    return 1;
  }
  ...
  if (last) {
    ...
    if ("at" in last) line += ` @ ${String(last.at)}`;
