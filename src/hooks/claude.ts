...
import { encodedCWD } from "../transcript/claudecode/claudecode.ts"
import { guardPath, sessionMatches } from "./guard.ts"
import {
  specFromProfile,
  type HookContext,
  type HookProvider,
...
// ClaudeHookPayload is the shape Claude Code delivers on a hook fire. Fields are
// optional because the parser tolerates partial payloads across event kinds.
...
const claudeStaticProfile: StaticHookProfile = {
...
function claudeSettingsPath(ctx: HookContext): string {
...
function validateTranscriptPath(ctx: HookContext, transcriptPath: string | undefined): string | null {
  if (!transcriptPath || ctx.cwd === "") return null
...
export function parseClaudeHookPayload(raw: string | ClaudeHookPayload, ctx: HookContext): ParsedEvent[] {
  ...
  if (!sessionMatches(ctx.harnessSessionID, sessionID)) return []
  ...
  if (payload.transcript_path !== undefined) {
    if (validateTranscriptPath(ctx, payload.transcript_path) === null) return []
  }
...
function buildEvents(payload: ClaudeHookPayload, sessionID: string): Event[] {
    ...
    case HookEventSessionStart:
      return [
        marker(
          EventSessionMeta,
          RoleSystem,
          `session-start:${payload.source ?? "startup"}`,
          sessionID,
...
function extractResponseText(resp: unknown): string {
  ...
  if (resp && typeof resp === "object") {
    const o = resp as { text?: unknown; content?: unknown }
    if (typeof o.text === "string") return o.text
    if (typeof o.content === "string") return o.content
    if (Array.isArray(o.content)) {
      let sb = ""
      for (const b of o.content as { type?: string; text?: string }[]) {
        if (b.type === "text" && typeof b.text === "string") sb += b.text
      }
      return sb
    }
  }
...
export class ClaudeHookProvider implements HookProvider {
