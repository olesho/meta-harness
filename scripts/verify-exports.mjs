...
if (targets.length === 0) {
  console.error("verify-exports: package.json has no `exports` — nothing to check");
  process.exit(1);
}

let failed = 0;
for (const { subpath, rel } of targets) {
  const spec = subpath === "." ? "meta-harness" : `meta-harness/${subpath.slice(2)}`;
  const url = pathToFileURL(join(root, rel)).href;
  try {
    const mod = await import(url);
    console.log(`OK  ${spec.padEnd(24)} ${Object.keys(mod).length} exports  (${rel})`);
  } catch (err) {
    failed++;
    console.error(`ERR ${spec.padEnd(24)} ${err?.code ?? ""} ${err?.message ?? err}`);
  }
}
